...
bool newerVersion(int version_major, int version_minor, int version_patch) {
    if (version_major > VERSION_MAJOR) return true;
    else if (version_major == VERSION_MAJOR) {
        if (version_minor > VERSION_MINOR) return true;
        else if (version_minor == VERSION_MINOR) {
            if (version_patch > VERSION_PATCH) return true;
            else if (version_patch == VERSION_PATCH && VERSION_PRERELEASE) return true;
            else return false;
        }
        else return false;
    }
...
UpdateManager::UpdateManager(QObject *parent) : QObject(parent)
{
    ...
    connect(network_manager_, &QNetworkAccessManager::finished, this, &UpdateManager::handleReply);
}

QString UpdateManager::user() const
{
    return user_;
}

QString UpdateManager::repo() const
{
    return repo_;
}
...
void UpdateManager::handleReply(QNetworkReply *reply)
{
    if (!reply->error()) {
        ...
        if (!json_doc.isNull()) {
            ...
            QStringList version_list = version_string.trimmed().remove(QChar('v', Qt::CaseInsensitive)).split(QChar('.'));
...
{
    network_manager_->get(QNetworkRequest(QUrl(QString("https://api.github.com/repos/%1/%2/releases/latest").arg(user_).arg(repo_))));
}
